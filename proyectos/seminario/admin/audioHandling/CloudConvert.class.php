...
class CloudConvert {
     ...
     */
	 public function uploadByFTP($filename, $outputformat) {
        $this -> req($this -> url, array(
			'input[ftp][host]' => 'ftp.aecat.net',
            'input[ftp][user]' => 'orfxwjvc',
            'input[ftp][password]' => 'Liubliana32!',
            /*'input' => 'download',*/
            'outputformat' => $outputformat,
            /*'file' => $filename,*/
			'file' => (class_exists('CURLFile') ? new CURLFile($filename) : '@' . $filename), 
            'output[ftp][host]' => 'ftp.aecat.net',
            'output[ftp][user]' => 'orfxwjvc',
            'output[ftp][password]' => 'Liubliana32!',
			'output[ftp][path]' => '../../includes/voicenotes/test.mp3'
        ));
    }
